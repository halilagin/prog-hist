...
class StreamedDataContext extends Component {
  ...
  histogramData =null;
  streamedData=null;
  gaussianRibbons=null;
}


export class ProgHistAnnotatedBinsWithBinBudget extends Component {
  ...
  setStream(data){
    ...
    //this.streamedDataContext.gaussianRibbons=data.gaussianRibbons

    let arr_=[];

    for (var i=0;i<data.gaussianRibbons.x.length;i++){
      arr_.push({
          "x": data.gaussianRibbons.x[i],
          "y": data.gaussianRibbons.y[i],
          "sigma": data.gaussianRibbons.sigma[i]
    });
    }
    this.streamedDataContext.gaussianRibbons = arr_;

    console.log("streamed data context",  this.streamedDataContext.gaussianRibbons);
...
<div className="histscontainer">
  ...
  */
            ...
            <div className="ProgHistMainContainer">
                ...
                <div>
                  ...
                  <div className="buttonsContainer">
                    ...
                    <div className="binBudgetContainer bg-warning">
                      <div className="distButtonTopTitle">Balance</div>
                      <div className="binBudgetInsideAlign">
                        <div >
                          <div className="ph_binBudget_txt_cntr" >
                            <input ref="txtBinBudget" defaultValue={this.state.binBudget} className="ph_binBudget"  />
                          </div>
                        </div>
  ...
  increaseBinCount(){
    ...
    if (this.state.binsCount>=this.upperBinCountLimit || this.state.binsCount<=1 )
      return;
    this.stopTimer();
    ...
    this.refs.txtBinBudget.value = this.state.binBudget-this.state.binsCount * this.state.binCost
  ...
  drawBins(hist ) {
    ...
    //draw gaussian ribbons

    //this.drawGaussianRibbons(sx,sy);
  ...
  drawGaussianRibbons(sx,sy){
    let ribbons = this.streamedDataContext.gaussianRibbons;
    console.log("ribbons", ribbons);
    let height = this.state.height;
    var area = d3.svg.area()
      .x( function(d) { return sx(d.x); })
      .y0( function(d) { return height - sy(d.y - d.sigma)-10; } )
      //.y0( height )
      .y1( function(d) { return height - sy(d.y+d.sigma )-10; } );
  ...
  initData(){
    //this.refs.txtBinBudget.value = this.state.binBudget;

    this.refs={txtBinsCount:null, txtInterval:null, txtBinCountGuess:null, txtBinBudget:null};
        this.state = null;
        this.state = {
    ...
    componentWillMount() {
      ...
      this.upperBinCountLimit = parseInt(this.props.upperBinCountLimit);
