...
function VarRadialBrush(container){
    ...
    for( var i=0;i<circleDatum.pointSize;i++) {
      ...
      if (higher>lower) {//normal scenario
        if (r >= lower && r < higher) {
          ...
          console.log("classed.idx", cls_, i);
        }
      ...
      console.log("range", r, lower, higher );
  ...
  this.projectPointsOnCircle = function (g_, data, dimName, center) {
      for (var i=0;i<data[0].length;i++) {
        ...
        g_.append("svg:circle")
          ...
          .attr("class", ("radial_brush ss_point ss_" + dimName + "_point_" + i) +  (" ss_" + dimName + "_radian_" + angle.radian))
  ...
  this.createRadialBrushData = function (g_, data, dimName){
  ...
  this.start = function(data, var1, var2) {
    this.data= data;
    this.createRadialBrushData(this.container, data, var1+"___"+var2);
    this.createRadialBrush(data, this.container, this.radialBrushConfig, var1+"___"+var2, 0);
  };
